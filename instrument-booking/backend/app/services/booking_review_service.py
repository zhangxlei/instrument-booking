...
async def create_review(db: AsyncSession, booking_id: uuid.UUID, reviewer_id: uuid.UUID | None = None) -> BookingReview:
    review = BookingReview(booking_id=booking_id, status="pending_review", reviewer_id=reviewer_id)
    db.add(review)
    await db.flush()
    return review
